...
@jwt_required()
def Toma_asistencia(id_maestro):
    current_user_id = get_jwt_identity()
    data = request.get_json()

    # Extracción de datos del JSON
    descripcion_materia = data.get('descripcion')
    id_alumno = data.get('alumno')
    valor_asistencia = data.get('valor')

    # Validación de datos
    if not descripcion_materia or not id_alumno or not valor_asistencia:
        return jsonify({"error": "Faltan datos"}), 400

    # Validación del token de inicio de sesión
    if current_user_id != id_maestro:
        return jsonify({"error": "Acción no permitida"}), 403
    ...
    try:
        cursor = connection.cursor()
        cursor.execute("""
            INSERT INTO asistencias (fecha, id_materia, id_alumno, asistencia)
            VALUES (NOW(), (SELECT id FROM materias WHERE nombre = %s LIMIT 1), %s, %s)
        """, (descripcion_materia, id_alumno, valor_asistencia))
        connection.commit()  # Asegurarse de hacer commit después de la inserción
        return jsonify({"message": "Asistencia capturada correctamente"}), 200
    except Exception as e:
